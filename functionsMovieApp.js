...
const pageTimerInterval = setInterval(() => {
    ...
    document.getElementById('page-time-time').textContent = 
        `${String(minutes).padStart(2, '0')}:${String(seconds).padStart(2, '0')}`; // Ensures the minutes and seconds are always displayed as two digits
}, 1000);
...
document.getElementById('start-pick-timer').addEventListener('click', () => {
  ...
  if (timerStatus === 'stopped') {
    const minutes = parseInt(document.getElementById('pick-a-movie-time-input').value);
...
const updatePickTimerDisplay = () => {
  ...
  document.getElementById('pick-a-movie-time').textContent = 
